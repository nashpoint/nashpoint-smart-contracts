...
- Asset-side pricing uses an external Chainlink-style oracle (e.g., fiat stablecoins) and only applies a staleness guard—not a deviation check—so feeds must be trusted/monitored offchain.
...
2. **Forward**: A manager batches requests using `forwardRequests`, which subscribes the aggregated assets via `subRedManagement`. Pending volume moves from `accumulatedDeposit` to `pendingDepositRequest` while the SubRedManagement contract holds the funds. `subRedManagement` is expected to consume the full allowance each call.
...
4. **Mint**: Each node completes the flow with `mint`, which requires minting the exact `maxMint` value. Any excess asset reimbursement recorded during settlement is returned before the adapter emits the canonical `Deposit` event. The `receiver` argument is kept for ERC7540 compatibility but the implementation enforces `msg.sender` as receiver/controller.
...
2. **Forward**: Managers relay redemptions to DigiFT via `forwardRequests`. Shares move from `accumulatedRedemption` to `pendingRedeemRequest` and are approved for the SubRedManagement contract.
...
- Additional checks enforce full execution (`MintAllSharesOnly`, `WithdrawAllAssetsOnly`) so reconciliations remain exact and `NothingToSettle` prevents settlement without pending volume. The `receiver` field is interface-only; actual transfers/mints go to `msg.sender`.
