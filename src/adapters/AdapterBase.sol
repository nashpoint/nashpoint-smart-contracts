    ...
    IERC7575
{
    ...
    event PriceUpdateDeviationChangeFund(uint64 oldValue, uint64 newValue);
    ...
    /// @notice Emitted when assets are deposited into fund
    /// @param assets The amount of assets subscribed
    event FundDeposited(uint256 assets);

    /// @notice Emitted when shares are redeemed from fund
    /// @param shares The amount of shares redeemed
    event FundRedeemed(uint256 shares);
    ...
    /// @notice The fund token address
    address public fund;

    /// @notice Number of decimals for the stToken
    uint8 internal _fundTokenDecimals;

    /// @notice Share price oracle
    IPriceOracle public fundPriceOracle;

    /// @notice Number of decimals for the share price oracle
    uint8 internal _fundPriceOracleDecimals;
    ...
    /// @notice Maximum time deviation for fund price updates (in seconds)
    uint64 public priceUpdateDeviationFund;

    /// @notice Maximum time deviation for asset price updates (in seconds)
    uint64 public priceUpdateDeviationAsset;

    /// @notice Last cached price from fund oracle
    uint256 public lastFundPrice;
    ...
    /**
     * @title InitArgs
     * @notice Initialization arguments for the Adapter
    ...
    struct InitArgs {
        ...
        /// @notice Address of the fund token
        address fund;
        /// @notice Address of the fund price oracle
        ...
        /// @notice Maximum time deviation for fund price updates (in seconds)
        uint64 priceUpdateDeviationFund;
        /// @notice Maximum time deviation for asset price updates (in seconds)
    ...
    function initialize(InitArgs calldata args) external initializer {
        ...
        // Set up fund and its oracle
        fund = args.fund;
        _fundTokenDecimals = IERC20Metadata(args.fund).decimals();
        fundPriceOracle = IPriceOracle(args.fundPriceOracle);
        _fundPriceOracleDecimals = IPriceOracle(args.fundPriceOracle).decimals();

        // Set deviation parameters
        priceDeviation = args.priceDeviation;
        settlementDeviation = args.settlementDeviation;
        priceUpdateDeviationFund = args.priceUpdateDeviationFund;
     ...
     */
    function setPriceUpdateDeviationFund(uint64 value) external onlyRegistryOwner {
        emit PriceUpdateDeviationChangeFund(priceUpdateDeviationFund, value);
        priceUpdateDeviationFund = value;
    }
    ...
    /**
     * @notice Get the current share price with validation
     * @dev Validates price deviation and staleness
     * @return The validated current price
     */
    function _getFundPrice() internal view returns (uint256) {
        (, int256 answer,, uint256 updatedAt,) = fundPriceOracle.latestRoundData();
        ...
        require(MathLib.withinRange(lastFundPrice, price, priceDeviation), PriceNotInRange(lastFundPrice, price));

        // Check if price data is not stale
        require(block.timestamp - updatedAt <= priceUpdateDeviationFund, StalePriceData(updatedAt, block.timestamp));
     ...
     */
    function settleDeposit(address[] calldata nodes, EventVerifierBase.OffchainArgs calldata verifyArgs)
        external
        nonReentrant
        onlyManager
    {
        (uint256 shares, uint256 assets) = _verifySettleDeposit(verifyArgs);
     ...
     * @param verifyArgs Offchain arguments for event verification
     */
    function settleRedeem(address[] calldata nodes, EventVerifierBase.OffchainArgs calldata verifyArgs)
        external
        nonReentrant
        onlyManager
    {
        (uint256 shares, uint256 assets) = _verifySettleRedeem(verifyArgs);
    ...
    /**
     * @notice Forward accumulated requests to fund protocol
    ...
    function forwardRequests() external onlyManager nonReentrant {
        ...
        if (pendingAssets > 0) {
            ...
            _fundDeposit(pendingAssets);
            emit FundDeposited(pendingAssets);
        }
     ...
     * @param fundPrice The current fund price
     * @return shares The equivalent number of shares
     */
    function _convertToShares(uint256 assets, uint256 assetPrice, uint256 fundPrice)
        internal
        view
        returns (uint256 shares)
    {
        uint256 num = MathLib.pow10(_fundTokenDecimals + _fundPriceOracleDecimals);
        uint256 den = MathLib.pow10(_assetPriceOracleDecimals + _assetDecimals);
        return assets.mulDiv(num, fundPrice).mulDiv(assetPrice, den);
    }
     ...
     * @return assets The equivalent amount of assets
     */
    function _convertToAssets(uint256 shares, uint256 assetPrice, uint256 fundPrice)
        internal
        view
        returns (uint256 assets)
    {
