...
contract FuzzSetup is HelperFunctions {
        ...
        registry = NodeRegistry(
            address(
                new ERC1967Proxy(
                    ...
                    abi.encodeWithSelector(NodeRegistry.initialize.selector, owner, protocolFeesAddress, 0, 0)
                )
            )
        );
    ...
    function _deployNode() internal {
        ...
        (node, escrowAddress) = factory.deployFullNode(
            NodeInitArgs("Test Node", "TNODE", address(asset), owner), payload, setupCalls, DEFAULT_SALT
        );
    ...
    function _getCurrentReserveRatio() internal view returns (uint256 reserveRatio) {
    ...
    function componentsByRouterForTest(address targetRouter) internal view returns (address[] memory matches) {
        if (address(node) == address(0)) {
