...
contract AaveVault is Test {
    ...
    function test_aave_integration() external {
        ...
        uint256 minAssetsOut = IERC4626(AAVE_HARVEST).previewRedeem(harvestShares) - 1;
        ComponentAllocation memory c = rwafi.getComponentAllocation(AAVE_HARVEST);

        // 2. Step
        vm.startPrank(RWAFI_OWNER);
        // set aave vault with the same config as harvest
        rwafi.addComponent(AAVE_VAULT, c.targetWeight, c.maxDelta, c.router);
        // harvest is set to zero
        rwafi.updateComponentAllocation(AAVE_HARVEST, 0, 0, c.router);
        ...
        router.liquidate(address(rwafi), AAVE_HARVEST, harvestShares, minAssetsOut);
