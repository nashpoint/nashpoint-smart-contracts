...
contract deJTRSYCentrifugeTest is Test {
    address root;
    ...
    address deJTRSY = 0x3a7d08B8B8E22251eFb112868A1640DF2FDBCb83;
    ...
    function setUp() public {
        vm.createSelectFork(vm.envString("ARBITRUM_RPC_URL"), 406315150);

        vm.startPrank(protocolOwner);
        erc7540Router.setWhitelistStatus(deJTRSY, true);
        vm.stopPrank();

        root = IAsyncVault(deJTRSY).root();
        manager = IAsyncRequestManager(IAsyncVault(deJTRSY).manager());
        globalEscrow = manager.globalEscrow();
        balanceSheet = IBalanceSheet(manager.balanceSheet());
        poolEscrow = balanceSheet.escrow(IAsyncVault(deJTRSY).poolId());
    }
    ...
    function test_deJAAA_integration() external {
        ...
        node.addComponent(deJTRSY, 50.31e16, 0, address(erc7540Router));
        vm.stopPrank();
        ...
        uint256 depositAmount = erc7540Router.investInAsyncComponent(address(node), deJTRSY);

        vm.stopPrank();

        vm.startPrank(root);
        manager.fulfillDepositRequest(
            IAsyncVault(deJTRSY).poolId(),
            IAsyncVault(deJTRSY).scId(),
            address(node),
            spoke.assetToId(IAsyncVault(deJTRSY).asset(), 0),
        ...
        balanceSheet.updateManager(IAsyncVault(deJTRSY).poolId(), address(manager), true);
        vm.stopPrank();
        deal(IAsyncVault(deJTRSY).share(), globalEscrow, depositAmount);

        vm.startPrank(rebalancer);

        uint256 sharesReceived = erc7540Router.mintClaimableShares(address(node), deJTRSY);

        erc7540Router.requestAsyncWithdrawal(address(node), deJTRSY, sharesReceived);
        ...
        poolEscrow.deposit(IAsyncVault(deJTRSY).scId(), IAsyncVault(deJTRSY).asset(), 0, uint128(depositAmount));
        poolEscrow.reserve(IAsyncVault(deJTRSY).scId(), IAsyncVault(deJTRSY).asset(), 0, uint128(depositAmount));
        deal(IAsyncVault(deJTRSY).asset(), address(poolEscrow), depositAmount);
        vm.stopPrank();

        vm.startPrank(rebalancer);
        uint256 assetsReceived = erc7540Router.executeAsyncWithdrawal(address(node), deJTRSY, depositAmount);
